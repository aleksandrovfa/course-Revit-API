...
namespace RevitAPITrainingElemBetweenPoint
{
    public class MainViewViewModel
    {
        ...
        private void OnSaveCommand()
        {
            ...
            if (ElementCount <= 0)
            {
                MessageBox.Show("Количестов элементов меньше либо равно 0");
                return;
            }
            ...
            {
                RaiseCloseRequest();
                Points = SelectionUtils.GetPoints(_commandData, "Выберите точки между которыми вставлять элементы", ObjectSnapTypes.Endpoints, 1);
                PointEl.Add(Points[0]);
            }
            else
            {
                RaiseCloseRequest();
                Points = SelectionUtils.GetPoints(_commandData, "Выберите точки между которыми вставлять элементы", ObjectSnapTypes.Endpoints, 2);
